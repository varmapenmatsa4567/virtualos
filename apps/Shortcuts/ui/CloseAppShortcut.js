...
const CloseAppShortcut = ({index, item, shortcutItems, setShortcutItems, removeItem}) => {
  ...
  return (
    <div draggable className="flex items-center group justify-between text-sm py-2 font-medium bg-[#2f2f30] p-2 w-2/3 rounded-xl border-2 border-[#464646]">
        <div className="flex items-center gap-2">
            ...
            {item.props.action == "app" && <select value={item.props.value} onChange={changeValue} className="rounded-md bg-[#2f343f] px-1 w-fit appearance-none text-[#007bff] text-center bg-transparent text-xs outline-none">
                {apps.map((app) => (
...
export default CloseAppShortcut;